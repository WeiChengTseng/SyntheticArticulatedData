...
def main(args):
    ...
    if not args.eval_only:
        ...
        scenegen.generate_scenes(args.n,
                                 args.obj,
                                 mean_flag=args.mean,
    ...
    scenegen.generate_scenes(int(args.n / 5),
                             args.obj,
                             test=True,
                             video=args.video)
    ...
    if not args.py_bullet and args.debug:
        make_animations(os.path.join(args.dir, args.obj),
                        min(100, args.n * 16),
                        use_color=args.debug)


parser = argparse.ArgumentParser(
    description="tool for generating articulated object data")
...
parser.add_argument('--masked',
                    action='store_true',
                    default=False,
                    help='remove background of depth images?')
parser.add_argument('--debug', action='store_true', default=False)
parser.add_argument('--mean',
                    action='store_true',
                    default=False,
                    help='generate the mean object')
parser.add_argument('--cute',
                    action='store_true',
                    default=False,
                    help='generate nice shots.')
parser.add_argument('--left-only',
                    action='store_true',
                    default=False,
                    help='generate only left-opening cabinets')
parser.add_argument('--py-bullet',
                    action='store_true',
                    default=False,
                    help='render with PyBullet instead of Mujoco')
parser.add_argument('--eval-only',
                    action='store_true',
                    default=False,
                    help='only generate evaluation dataset')
parser.add_argument(
    '--video',
    action='store_true',
    default=False,
    help='generate video in addition to images (in the form of png sequences)')
