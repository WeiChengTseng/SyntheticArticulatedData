...
def make_animations(dir, nframes, use_color=False):
    ...
    fig = plt.figure()
    ax = plt.axes()
    x0 = torch.load(os.path.join(dir, 'depth') + str(0).zfill(6) +
                    '.pt').numpy()
    img = ax.imshow(x0, cmap='gray')

    # animation function.  This is called sequentially
    def animate(i):
        x = torch.load(os.path.join(dir, 'depth') + str(i).zfill(6) +
                       '.pt').numpy()
        img.set_data(x)
        return img,
    ...
    anim = animation.FuncAnimation(fig,
                                   ...
                                   interval=20,
                                   blit=True)
    anim.save(os.path.join(dir, 'animation.mp4'),
              fps=30,
    ...
    if use_color:
        ...
        anim = animation.FuncAnimation(fig,
                                       ...
                                       interval=20,
                                       blit=True)
        anim.save(os.path.join(dir, 'color_animation.mp4'),
                  fps=30,
                  extra_args=['-vcodec', 'libx264'])
        # plt.show()
